...
@app.route('/')
def index():
    # Serve the static index.html file from root directory
    try:
...
def api_products():
    ...
    if search_query:
        # Search for products by name using LIKE
...
def export_orders():
    ...
    # Get filter/search parameters (same as /orders)
    customer_id = request.args.get('customer_id')
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')
    search = request.args.get('search')
    ...
    writer.writerow(['Order Code', 'Customer', 'Items', 'Total', 'Order Date'])
    for order in orders:
        writer.writerow([order['code'], order['customer'], order['items_summary'], order['total'], order['order_date']])
    output.seek(0)
    return send_file(io.BytesIO(output.getvalue().encode()), mimetype='text/csv', as_attachment=True, download_name='orders.csv')
